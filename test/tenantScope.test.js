...
// Seed (IDs sentinela altos), dentro da transação; tudo desfeito no rollback.
// tenant 900001: gestor 900010, usuario 900011 (dono de __t_i2__), usuario 900012 (dono de __t_i1__)
// tenant 900002: usuario 900013 (dono de __t_i3__)
// equipe 900100 (t1): gestor 900010 gerencia; membro = 900012
async function seed(conn) {
  ...
  await conn.query(`INSERT INTO users (id, tenant_id, name, email, password_hash, role) VALUES
    ...
    (900012,900001,'User2','u2@__test__','x','usuario'),
    (900013,900002,'User3','u3@__test__','x','usuario')`);
  await conn.query(`INSERT INTO sentinela_instances (id, tenant_id, owner_user_id, name, token) VALUES
    ('__t_i1__',900001,900012,'A','t1'),
    ('__t_i2__',900001,900011,'B','t2'),
  ...
  await conn.query("INSERT INTO team_users (team_id, user_id) VALUES (900100,900012)");
}
...
describe('visibleInstanceIds (owner/membro, em transação com rollback)', () => {
  ...
  it('gestor vê as instâncias dos usuários-membros das equipes que gerencia', async () => {
    await withTx(async (conn) => {
      await seed(conn);
      const ids = await visibleInstanceIds(conn, { role: 'gestor', tenantId: 900001, userId: 900010 });
      expect(ids).toEqual(['__t_i1__']); // dono é o membro 900012
    });
  });
  it('usuario vê só as próprias (owner = ele)', async () => {
    await withTx(async (conn) => {
      await seed(conn);
