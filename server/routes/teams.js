...
export function createTeamsRouter(pool) {
  ...
  router.get('/:id/instances', async (req, res) => {
    try {
      ...
      if (!team) return res.status(404).json({ error: 'Equipe não encontrada' });
      const [rows] = await pool.query(
        `SELECT DISTINCT si.id, si.name, si.status, si.owner_user_id, u.name AS owner_name
         FROM team_users tu
         JOIN sentinela_instances si ON si.owner_user_id = tu.user_id
         JOIN users u ON u.id = si.owner_user_id
         WHERE tu.team_id = ? ORDER BY si.name`, [team.id]);
      res.json(rows.map((r) => ({ id: r.id, name: r.name, status: r.status, ownerUserId: r.owner_user_id, ownerName: r.owner_name })));
    } catch (e) {
      console.error('list team instances (derived):', e);
      res.status(500).json({ error: 'Falha ao listar números da equipe' });
    }
  });
